...
class BitMapToKaoSwapManager {
  ...
  public:
	...
	//				┌CDデフォ。1回も未起動。　┌通常のKaoSwap.n6pサイズ
	enum FileSile { DefaultKaoSwapSize=339854, NormalKaoSwapSize=2558772 };
	

  private:
	char szBMPFaceFileName[32];		// 0000.bmp 0000は番号 0001スタート
	char szBMPFaceFileFormat[32];   // ".\\kao_3bit\\%04d.bmp"
  ...
  private:
	...
	char szKaoswapFileName[32];						// KAOSWAPのファイルネーム
  ...
  public:
	BitMapToKaoSwapManager(char* szKaoSwapName) {
		...
		// フラグ部分は全て１に仕立てなおす
		memset( dstKaoSwap.flag, 0x01, BUSHOU_NUM );

		// ビットマップフォーマットの命名パターン。ここで文字直接埋め込みってどうなの…
		strcpy_s(szBMPFaceFileFormat, ".\\kao_3bit\\%04d.bmp");
	...
	ErrorType ReadKaoSwap() {
		...
		// ファイルサイズが小さく、CDデフォのサイズならエラー値を返しておく。
		if ( fileSize == DefaultKaoSwapSize ) {
			cout << ":" << "ErrUnUsedKaoSwapSize" << endl;
			getchar();
	...
	ErrorType OpenBmpFile(int iFileNo) {
		...
		// 番号→ファイル名への変換
		wsprintf(szBMPFaceFileName, szBMPFaceFileFormat, iFileNo);
	...
	ErrorType CnvBmpToKaoswap(int iFileNo) {
		// 顔SWAPの顔利用フラグをちゃんとONにする。
		dstKaoSwap.flag[iFileNo-1] = 0x01; // まずその武将は居たということとなる。
